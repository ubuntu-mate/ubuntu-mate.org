...
function do_rpi2() {
    ...
    local ISO_PATH="${HOME}/ISO/vivid/armhf/ubuntu-mate-15.04-desktop-armhf-raspberry-pi-2.img.bz2"
...
function do_rootfs() {
    ...
    local ISO_PATH="${HOME}/ISO/vivid/armhf/ubuntu-mate-15.04-desktop-armhf-rootfs.tar.gz"
    local ISO_FILE=$(basename ${ISO_PATH})
    local ISO_SIZE=$(du -m ${ISO_PATH} | cut -f1 -s)
    #local TOR_PATH=""
    #local TOR_FILE=$(basename ${TOR_PATH})
